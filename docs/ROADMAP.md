...
- P0-17 用 Android TV UI 渲染服务器输入、登录、首页、详情和播放器入口。进度：原生 View 最小流程已完成，服务器 URL 输入、密码遮蔽、服务器连接、登录、会话恢复、首页加载、详情加载、继续播放、从头播放、上下文错误恢复、Back 导航和播放准备均走已接线流程，尚未做最终 TV 视觉打磨。

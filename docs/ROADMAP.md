...
- public-user 登录界面已具备最小入口、passwordless 一键确认，以及“需要密码 / 免密码登录”状态标识；后续可补头像。
...
- 设备端诊断信息已具备最小快照、不含 token 的文件导出和系统文本分享；错误消息里的播放 URL token、access token query 和 token header 会统一脱敏。

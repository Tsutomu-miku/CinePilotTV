...
- P0-9 通过直接 URL 实现服务器发现。进度：请求规格和系统信息响应 mapper 已完成，真实 HTTP transport 未开始。
- P0-10 实现 Jellyfin 与 Emby 的用户名 / 密码认证。进度：请求规格和登录响应 mapper 已完成，真实 HTTP transport 未开始。
...
- P0-13 获取播放信息，并保存 media source id 与 play session id。进度：playback info 请求规格、领域模型和响应 mapper 已完成。

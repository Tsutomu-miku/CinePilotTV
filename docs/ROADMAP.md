...
Infuse 风格改造优先按 `docs/INFUSE_DESIGN_SPEC.md` 执行：当前结论是“换主题色 / 半透明色块”属于失败路径；近期最高优先级改为 artwork-driven shell，也就是焦点媒体驱动背景、真实 glass primitive、首页 artwork hub、详情 cinematic backdrop 和播放器周边 glass overlay。播放器控制层暂不重写。
...
- 首页工具区精简与可理解性：搜索、刷新、切换账号、设置、退出已使用清晰图标和紧凑按钮；按钮顺序按使用频率排列，高风险“退出”放在最后。
- 首页全局设置入口：已在首页工具区提供“设置”图标按钮，进入全局设置页；当前支持主题选择，选择后立即应用并保存到本机。
...
- 首页 Infuse 化结构：已引入 focused artwork hub，焦点媒体会驱动背景 Backdrop / Thumb / Primary fallback 和左侧摘要；海报卡常态只展示 artwork，焦点卡才显示轻玻璃标题，顶部工具区退为轻量 rail。
...
- 详情页 cinematic backdrop：已用媒体 Backdrop / Thumb / Primary fallback 作为全屏背景，主决策区浮在背景上，音轨 / 字幕 / 技术信息和简介进入共享 glass 区。
- 详情页轨道选择密度：媒体源、音轨和字幕已使用类似 Jellyfin / Emby 的摘要行 + popover OptionSelect，保持短路径，同时避免把所有候选项平铺在详情页。
...
- 详情页焦点滚动：已把播放按钮、媒体源、音轨和字幕 OptionSelect 摘要行接入统一垂直 D-pad 滚动兜底；选择后焦点应停留在刚操作的摘要行，继续在真机上校准从标题到播放按钮、从播放按钮到选择器、从选择器回到标题的自然滚动。

### UI-3 播放器与错误恢复

- 播放器基础体验：播放器保持黑底全屏，OK / Enter 直接暂停或恢复，暂停时呼出 Media3 控制层并把焦点放到原生播放 / 暂停图标；控制层隐藏时左右键直接 `-30s / +30s`，并呼出时间轴与轻量 `+30 秒 / -30 秒` 反馈，连续左右键仍保持快捷 seek；Back 使用 Toast 二次确认；Media3 控制层不展示重复的快退和快进按钮，也不叠加第二套 app 级播放控制。
...
- 全局设置入口：首页工具区展示“设置”，不藏在退出或账号切换流程里。
- 主题设置：设置页提供主题 OptionSelect，当前包含 CinePilot 青绿、Infuse 玻璃、Jellyfin 蓝和 Emby 绿；选择后立即应用到完整 palette，包括背景、surface、输入框、海报 fallback、焦点色、强调色、继续播放色、文字色、pill 边框和 overlay，并保存到本机。
- 后续收敛方向：字幕默认样式、默认播放策略、调试信息显示偏好可以进入全局设置，但每一项都需要保持 TV 遥控器可扫读，不做密集手机表单。

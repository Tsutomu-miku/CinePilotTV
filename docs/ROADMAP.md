...
- P0-11 按服务器和用户作用域持久化会话。进度：内存 repository、文件 repository、client 保存 / 恢复 / logout / 本地忘记撤销流程、Android 稳定 device id、Android 最近登录列表、最近服务器地址、启动自动恢复最近账号和 TV 退出登录入口已完成；401 过期会话会清除当前 scope 并回到重新登录。
...
- P0-16 把播放暂停、seek、音轨、字幕和播放速度变化同步到协议层。进度：立即上报事件调度、client 发送、默认音轨 / 字幕 index、播放前音轨 / 字幕 / 播放速度选择、`PlaybackSessionController`、Media3 ready / pause / unpause / seek / ended / release / playback speed 桥接已完成；播放中音轨和字幕变化仍需在能可靠映射 Jellyfin / Emby `MediaStream.Index` 后接入。
...
- 设备端诊断信息已具备最小快照、不含 token 的文件导出和系统文本分享。
...
- Live TV 支持。
- 离线诊断导出已具备最小入口和系统文本分享，后续可扩展为更完整的系统文件分享。
- 更丰富的 codec 兼容性报告。

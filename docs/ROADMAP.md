...
- 显式选择的字幕如果无法作为外部 `DeliveryUrl` 附加给 Media3，播放源选择会改走服务器 HLS / transcode 并带上字幕参数，避免 direct play 路径丢字幕或让播放器 CC 图标无可选轨道。
...
- 播放器视频信息：播放器页新增小型“视频信息”按钮，默认只占右上角少量空间；展开后展示播放方式、媒体源、URL 类型、请求路径、视频 / 音轨 / 字幕和字幕交付方式，方便排查直连、转码和字幕问题。

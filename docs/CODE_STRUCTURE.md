...
- Android 视觉样式和可复用 View helper 放在 `app/.../tv/ui`；登录相关页面放在 `app/.../tv/auth`；详情页组装放在 `app/.../tv/details`；播放准备、音轨字幕、速度和诊断页面放在 `app/.../tv/playback`；页面流程仍在 Activity，后续继续按 home 拆分，尽量让单文件保持在 300 行左右。

...
- 真机“音轨 / 字幕”入口已验证可以列出音轨、关闭字幕和多个字幕 stream，例如外挂中文字幕与英文字幕，并可把选择带入播放准备。当前流程还支持先选择音轨，再在同一媒体源下选择服务器默认字幕、关闭字幕或具体字幕，让音轨和字幕作为一组偏好进入播放准备。
...
- 详情页和主页按钮获得遥控器焦点时必须高对比显示，包含亮色填充、内部焦点环和短过渡动画，不能只依赖细描边或颜色微调。
...
- 播放失败后的错误页应提供“切换音轨 / 字幕”入口，直接进入播放选项页，方便处理音轨、字幕或媒体源导致的失败。

...
- 播放失败后的错误页应提供“诊断信息”入口，可导出或分享不含 token 的诊断快照，并能返回错误页继续选择恢复动作；即使底层错误消息带有 `api_key`、access token query 或 token header 片段，诊断文本也必须先脱敏。错误页打开后，第一个可用恢复动作必须自动获得遥控器焦点。
- 诊断页“导出诊断”应使用下载图标、主按钮样式和初始焦点；“分享诊断”应使用分享图标。导出完成页应把“分享诊断”作为主按钮和初始焦点。

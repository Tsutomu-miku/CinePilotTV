...
- 真机曾验证可列出音轨、关闭字幕和多个字幕 stream，例如外挂中文字幕与英文字幕，并可把选择带入播放准备。当前流程已调整为详情页内联 OptionSelect：在同一媒体源下选择服务器默认 / 指定音轨、服务器默认 / 关闭 / 指定字幕，让音轨和字幕作为一组偏好进入播放准备。
...
- 详情页的音轨、字幕和媒体源选择应以内联 OptionSelect 呈现；选择只更新播放偏好，不应跳到单独的“音轨 / 字幕”页面。媒体源、音轨和字幕选项应使用统一可换行设置组容器，不能退回为散落的裸单选项，也不能退回为带圆点的 RadioSelect 样式。
- 详情页播放按钮获得焦点时，遥控器下键必须能进入下方内联音轨 / 字幕 OptionSelect；从下方返回播放按钮后，继续按上键必须能滚回标题和元信息，不能被防丢焦逻辑拦截。媒体源、音轨和字幕选项也必须使用同一套垂直 D-pad 滚动兜底：有相邻焦点时移动焦点，已经到达焦点边缘时滚动页面。
...
- 播放失败后的错误页应提供“切换音轨 / 字幕”入口，返回带内联单选框的详情页，方便处理音轨、字幕或媒体源导致的失败；该入口不应依赖 playable media 已经创建。
...
- 详情页的音轨 / 字幕内联 OptionSelect 能按 media source 分组列出 playback info 中的 media sources、audio streams 和 subtitle streams，并把选择所属的 media source id / stream index 一起用于播放准备，避免多版本影片选错源；如果 playback info 重新加载后旧 media source 或 stream index 已不存在，界面和播放准备都必须回退到服务器默认选择。

...
- 首页媒体卡片在 1920x1080 / 320dpi 电视上不应只显示一两个巨大海报；首屏应能看到多个媒体库卡片，焦点卡片必须有粗焦点环、亮色标题底条和轻微放大。
- 详情页和主页按钮获得遥控器焦点时必须高对比显示，包含亮色填充、粗焦点环和轻微放大，不能只依赖细描边或颜色微调。
...
- 详情页必须提供“字幕样式”入口；字号、颜色和背景选择会保存到本机，并在下一次创建 Media3 播放器时应用到字幕渲染。

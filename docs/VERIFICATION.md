...
- 搜索页应提供全部、电影、剧集、单集和视频筛选；切换筛选后应保留当前输入词，提交后结果标题应展示当前筛选语义。
...
- 详情页“播放速度”应使用横向紧凑选项组，默认焦点落在 1.0x；选择 0.75x、1.0x、1.25x、1.5x、2.0x 后应准备播放，并由 Media3 应用选中速度，同时播放上报保留 `PlaybackRate`。如果用户已在详情页选择媒体源、音轨或字幕，通过播放速度入口进入播放也必须保留这些选择。

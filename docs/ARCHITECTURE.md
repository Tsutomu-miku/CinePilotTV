...
播放准备过程不展示 raw playback URL，也不要求用户停在“打开播放器”的二次确认页；详情页动作准备完成后应直接进入 Media3 播放器。需要排障时走 `TvDiagnostics`，且诊断输出不能包含 token。Android 诊断导出和系统分享由 `PlaybackDiagnosticsController` 编排，只能写入 app 私有文件；分享时通过 `FileProvider` 授权一个临时只读 `content://` 文件 URI，并附带文本正文作为 fallback，避免无意把 token、播放 URL 或真实私有路径暴露给其它应用。
...
Jellyfin / Emby 播放协议、HLS 参数和字幕策略的项目准则见 `docs/JELLYFIN_PLAYBACK_GUIDE.md`；渐进式 API 速查见 `docs/JELLYFIN_API_REFERENCE.md`。播放链路改动前必须先对照这两份文档，避免跳过 playback info 协商直接手写 HLS 参数。

进入播放器页后，Activity 应主动把焦点交给 `PlayerView`，让遥控器播放控制优先落在 Media3。播放器页面必须使用黑底全屏播放器 surface，不叠加第二套 app 级播放按钮；播放 / 暂停、seek、进度条和控制显隐交给 Media3 原生控制层与遥控器媒体键。暂停态应聚焦原生播放 / 暂停图标并允许方向键进入时间轴，退出播放使用系统 Back。
...
Media3 播放速度变化通过 `onPlaybackParametersChanged` 上报到 `PlaybackSessionController.playbackRateChanged`。`PlayableMedia` 必须携带当前媒体源的 `MediaStreamInfo` 列表，`Media3StreamIndexResolver` 只能用 label、language、codec 这些稳定 metadata 做唯一匹配；音轨和字幕变化只有在能从 Media3 selected track 稳定映射回服务器 `MediaStream.Index` 时才应上报，避免把本地 track ordinal 错当协议 stream index。字幕被关闭时沿用播放偏好语义上报 `SubtitleStreamIndex=-1`。
